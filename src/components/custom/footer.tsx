...
import { FooterProps, SocialLink } from "@/types/footertypes";
import { LogoText } from "@/types/headertypes";
import { getsociallinksurl } from "@/lib/strapi-imagehelper";
...
export function Footer({ data }: {data : FooterProps}) {
  const { logoText, socialLink, text } : {logoText : LogoText, socialLink : SocialLink[], text : string}= data;
  return (
    <div className="dark bg-gray-900 text-white py-8">
      <div className="container mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between">
        <Logo dark url={logoText.url} text={logoText.text} />
          ...
          {socialLink.map((link : SocialLink) => {
            return (
              <Link
                ...
                href={getsociallinksurl(link.url)}
                key={link.id}
                replace
              >
